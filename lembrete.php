<?php
   ...
   @mysqli_select_db($conexao, $banco);//selecione o banco de dados
   ...
   switch($dialet){//verifica que dia cai
	   ...
	   for($i = 1; $i <= $n; $i++ ){/*agora vamos no banco de dados verificar os evendos*/
		   ...
		   $sqlag = mysqli_query( $conexao, "SELECT * FROM agenda WHERE dtevento = '$dtevento'") or die(mysqli_error($conexao));
   ...
   if(isset($_GET['d'])){/*link dos dias de eventos*/
	   $idev = $_GET['d'];
	   $sqlev = mysqli_query("SELECT * FROM agenda WHERE dtevento = '$idev' ORDER BY hora ASC") or die(mysqli_error($sql));
